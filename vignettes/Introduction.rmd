...
```batchCorr``` is designed to work with basic data classes. Adundances are 
included in a matrix, while sample and feature metadata is included in a 
data.frame. ```batchCorr``` works best as per the chronology presented below. 
...
- ```driftCalc```accepts arguments specifying the smoothing function 
(LOESS or cubic spline)
